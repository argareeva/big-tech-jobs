...
**Known accepted trade-off:** dropping the adjacency requirement means some now-broader matches are not true APM/PM roles — e.g. "Senior Associate, Product Management" and "Associate Director of Product Marketing" used to match, even though they're mid/senior or marketing-adjacent titles, not entry-level PM programs. This was surfaced to the user rather than silently re-narrowed.

**Update (2026-09-08, Layer 1 tightened):** added seniority and marketing-role exclusions in `isApmTitle` — when the Layer 1 qualifier+product/program pattern matches, it is rejected for director/vp/vice president/chief/senior titles and for Product Marketing/marketing-role titles. An explicit Associate Product Manager title remains eligible when marketing describes its product domain (for example, StubHub's Marketing Operations and Automation role). Layer 2 (per-company `titleAliases`) and the PayPal-GBLP/bare-APM branches are unaffected.
