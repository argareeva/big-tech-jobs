...
**Update (2026-09-08, Layer 1 tightened):** added `SENIORITY_OR_MARKETING_EXCLUSION_RE` in `isApmTitle` — when the Layer 1 qualifier+product/program pattern matches, it's rejected anyway if the title also contains director/vp/vice president/chief/senior/marketing. This does not reinstate the old adjacency rule; it only carves out the specific false positives above. Layer 2 (per-company `titleAliases`) and the PayPal-GBLP/bare-APM branches are unaffected by this exclusion.
