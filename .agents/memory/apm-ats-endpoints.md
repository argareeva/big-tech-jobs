...
| Mastercard | Workday | Host mastercard.wd1.myworkdayjobs.com, tenant CorporateCareers (Phenom People CMS skin over Workday, found via a live job page's apply link). Default searchText "associate product manager" doesn't rank "Associate Product Specialist, Product Management" into its top 20 — needed searchText override "associate product" — found 2026-09-08. |
| Palo Alto Networks | Workday | Host paloaltonetworks.wd5.myworkdayjobs.com, tenant panwexternalcareers — found via jobs.paloaltonetworks.com referencing the tenant directly, confirmed live 2026-09-08. searchText widened to "product management" (default "associate product manager" phrase would miss "Product Management Academy" wording); 0 current Academy postings is a real seasonal gap — the search itself returns hundreds of real unrelated jobs, not an error. |
| Experian | SmartRecruiters | `boardSlug = "Experian"` — jobs.experian.com (plain fetch of experian.com/careers returns 403 Cloudflare, but jobs.experian.com redirects past it) 301s to a SmartRecruiters-backed board; identifier confirmed via a live apply-link. Confirmed live 2026-09-08, 450 total postings. |
| Apple | Custom | `POST https://jobs.apple.com/api/v1/search` (not `/api/v1/search/search` — a wrong path guess there produces 401s that look like an auth lock but are really a 404-shaped wrong endpoint). Needs a session: GET the search page for a `jobs` cookie, GET `/api/v1/CSRFToken` for a token (send back as `X-Apple-CSRF-Token`), then POST with body `{query, filters, page, locale, sort, format}` — no login/candidate auth required. Search is relevance-ranked like Workday; wrap the query in literal double quotes (e.g. `"product manager"`) to narrow results instead of thousands of unrelated retail postings. Paginated (20/page, sorted newest-first) — fetch every page up to `totalRecords`, not just page 1, or a real match can silently drop past the first page. Found by reading the site's own JS bundle (`jobsite.main.*.js`) for its request-config object and payload-builder function, not by guesswork. |
| Jane Street | Custom | `GET https://www.janestreet.com/jobs/main.json` — no auth, no company/keyword filter server-side, returns every open posting (232 as of 2026-09-08). Found by fetching the open-roles page's own JS bundle and grep'ing for a `.json` reference. **Internship signal is NOT in the title text** — e.g. "Strategy and Product" is used for both a Summer Internship and (as "Strategy and Product Specialist") the full-time role; must filter on the separate `availability` field ("Summer Internship" vs "Full-Time: ...") instead of relying on title-text internship detection. Apply URL pattern: `https://www.janestreet.com/join-jane-street/position/{id}/`. |
